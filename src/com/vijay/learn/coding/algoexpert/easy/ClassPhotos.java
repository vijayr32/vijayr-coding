...
public class ClassPhotos {
  public boolean classPhotos(ArrayList<Integer> redShirtHeights, ArrayList<Integer> blueShirtHeights) {
    ...
    int len = blueShirtHeights.size();
    boolean isFirstRed = redShirtHeights.get(0) < blueShirtHeights.get(0) ? true : false;
    for (int i=1; i<len; i++) {
      if (isFirstRed) {
...
/*
Difficulty: Category: HiddenSuccessful Submissions: 22,687+Class Photos
...
Optimal Space & Time ComplexityO(nlog(n)) time | O(1) space - where n is the number of students
 */
