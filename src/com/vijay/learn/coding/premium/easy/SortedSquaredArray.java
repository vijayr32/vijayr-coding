...
Optimal Space & Time ComplexityO(n) time | O(n) space - where n is the length of the input arrayInstructor: Tim RuscicaGo to Conceptual OverviewGo to Code Walkthrough
 */
...
public class SortedSquaredArray {
  ...
  public int[] sortedSquaredArray(int[] array) {
    ...
    while (i<=j) {
      int prodI = array[i]*array[i];
      int prodJ = array[j]*array[j];
      if (prodI > prodJ) {
        arr[a] = prodI;
        i++;
      }
